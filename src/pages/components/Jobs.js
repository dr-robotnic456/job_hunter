...
const Jobs = ({ result, handleClick, category, isActive, setIsActive }) => {
  if (result && result.length === 0) {
    return (
      <div>
        <div id='job' className='bg-white flex-col flex h-screen  flex-wrap w-full overflow-hidden max-w-screen-lg flex-start gap-1 dark:bg-black'>
    ...
    return (
      <div id='job' className='bg-white flex-col h-screen flex flex-wrap  w-full overflow-hidden max-w-screen-lg flex-start gap-1 dark:bg-black'>
        <CategoryCard handleClick = {handleClick} category = {category} isActive={isActive} setIsActive={setIsActive}/>
        <div className="flex">
          {result}
        </div>
